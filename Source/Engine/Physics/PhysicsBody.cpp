...
namespace STR_FALL
{
	PhysicsBody::PhysicsBody(const Transform3D& transform, const Vector2& size, const PhysicsBodyDef& def, const Physics& physics) 
	{
		...
		// create shape
		b2Vec2 hsize = Physics::ToB2(Physics::PixelToWorld(size * transform.m_scale * 0.5f));
		switch (def.shape)
		{
		case Shape::Box:
		{
